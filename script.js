// =========================================
// 우리의 기록장 - 최종 정리본 script.js
// =========================================

const STORAGE_KEYS = {
    welcomeSeen: "memorySiteWelcomeSeen",
    theme: "memorySiteTheme",
    visitCount: "memorySiteVisitCount"
};
...
function initWelcomeModal() {
    ...
    const hasSeenWelcome = safeStorageGet(STORAGE_KEYS.welcomeSeen) === "true";
...
function closeWelcomeModal() {
    ...
    safeStorageSet(STORAGE_KEYS.welcomeSeen, "true");
}
    ...
    if (inputHash === savedPasswordHash) {
        ...
        setTimeout(() => {
            typeWriterEffect(letterTextDiv, originalHTML, 20, () => {
                ...
                replyBtn.animate([{ opacity: 0 }, { opacity: 1 }], {
                    duration: 1000,
                    fill: "forwards"
                });
            });
...
function initLightbox() {
    ...
    galleryImages.forEach((img) => {
        ...
        img.addEventListener("click", function () {
            ...
            lightboxImg.alt = this.alt || "확대된 사진";
...
function createStar(x, y) {
    const now = Date.now();
    if (now - lastTrailTime < 24) return;
    lastTrailTime = now;
...
window.addEventListener("scroll", function () {
    ...
    targetScrollPercent = Math.min(100, Math.max(0, (scrollTop / docHeight) * 100));
...
function smoothScrollAnimation() {
    ...
    if (scrollBar) scrollBar.style.width = `${currentScrollPercent}%`;
...
// 첨부파일/로컬 파일 오류 보정
// =========================================
function initAttachmentFallbacks() {
    document.querySelectorAll("img").forEach((img) => {
        img.addEventListener("error", function () {
            const inlineFallback = getFallbackFromOnError(this.getAttribute("onerror"));
            if (inlineFallback && this.src !== inlineFallback) {
                this.src = inlineFallback;
                return;
            }
            this.classList.add("asset-fallback-image");
            this.alt = this.alt || "이미지를 불러오지 못했습니다";
        });
    });
    ...
    if (audio) {
        audio.addEventListener("error", function () {
            document.body.classList.remove("bgm-playing");
            document.getElementById("bgm-container")?.classList.remove("playing");
            document.getElementById("bgm-icon")?.classList.remove("rotating");
            showSecretToast("BGM 파일을 불러오지 못했어. mp3 파일을 HTML과 같은 폴더에 두거나, 인터넷 연결을 확인해줘.", 4200);
        }, true);
    }
}
...
// 사진 슬라이드쇼
// =========================================
function getFallbackFromOnError(onerrorText) {
    if (!onerrorText) return "";
    const match = String(onerrorText).match(/this\.src=['"]([^'"]+)['"]/);
...
function initSlideshow() {
    ...
    slides = Array.from(galleryItems).map((item) => {
        ...
        return {
            ...
            fallback: getFallbackFromOnError(img?.getAttribute("onerror")),
    ...
    if (slideImage) {
        ...
        setTimeout(() => {
            slideImage.onerror = function () {
                if (currentSlide.fallback && this.src !== currentSlide.fallback) {
                    this.src = currentSlide.fallback;
                } else {
                    this.onerror = null;
                }
            };
...
function initMobileNavActiveState() {
    ...
    function updateActiveByScroll() {
        ...
        sections.forEach(section => {
            if (checkLine >= section.offsetTop - 40) {
                activeId = section.id;
            }
        });
...
function initThemeSwitcher() {
    const savedTheme = safeStorageGet(STORAGE_KEYS.theme) || "night";
...
function updateVisitCount() {
    ...
    const currentCount = Number(safeStorageGet(STORAGE_KEYS.visitCount) || 0) + 1;
    safeStorageSet(STORAGE_KEYS.visitCount, String(currentCount));
...
function initEasterEggs() {
    ...
    document.addEventListener("keydown", (event) => {
        const editable = event.target.closest?.("input, textarea, [contenteditable='true']");
        if (editable || event.ctrlKey || event.metaKey || event.altKey || event.key.length !== 1) return;
...
// 엔딩 크레딧
// =========================================
function prepareEndingCreditMetrics() {
    const credits = document.getElementById("ending-credits");
    const mask = credits?.querySelector(".credits-mask");
    const roll = document.getElementById("credits-roll");
    if (!credits || !mask || !roll) return false;

    credits.classList.remove("finished");
    roll.style.animation = "none";
    roll.style.transform = "translateY(0)";

    const maskHeight = Math.max(mask.clientHeight || 0, window.innerWidth <= 768 ? 300 : 340);
    const rollHeight = Math.max(roll.scrollHeight || 0, 1);
    const from = maskHeight + 32;
    const to = -(rollHeight + 32);
    const distance = Math.abs(from - to);
    const duration = Math.min(68, Math.max(28, distance / 34));

    roll.style.setProperty("--credits-from", `${from}px`);
    roll.style.setProperty("--credits-to", `${to}px`);
    roll.style.setProperty("--credits-duration", `${duration}s`);

    // reflow 후 CSS animation이 다시 적용되도록 초기화합니다.
...
function playEndingCredits(options = {}) {
    const credits = document.getElementById("ending-credits");
    const roll = document.getElementById("credits-roll");
    ...
    credits.classList.remove("play", "finished");
    if (!prepareEndingCreditMetrics()) return;
...
function initEndingCredits() {
    ...
    roll.addEventListener("animationend", function (event) {
        if (event.animationName !== "creditsRollSafe") return;
        credits.classList.remove("play");
        credits.classList.add("finished");
    });
    ...
    window.addEventListener("resize", function () {
        if (credits.classList.contains("play")) {
            playEndingCredits();
        }
    });
}
